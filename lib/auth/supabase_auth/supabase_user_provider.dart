...
class RupiesEmpresasSupabaseUser extends BaseAuthUser {
  RupiesEmpresasSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> rupiesEmpresasSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = RupiesEmpresasSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
